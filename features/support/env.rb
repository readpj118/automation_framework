...
Before do
  ...
  if TestConfig['headless']
    caps = Selenium::WebDriver::Remote::Capabilities.chrome("chromeOptions" => {"args" => %w(--start-maximized --headless)})
  else
    caps = Selenium::WebDriver::Remote::Capabilities.chrome("chromeOptions" => {"args" => %w(--start-maximized)})
  end
